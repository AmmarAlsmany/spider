...
class PesticideAnalyticsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Get all places for the filter
        $places = \App\Models\branchs::orderBy('branch_name')->get();

        // Default to last 30 days if no date range is specified
        $startDate = $request->input('start_date', Carbon::now()->subDays(30)->format('Y-m-d'));
        $endDate = $request->input('end_date', Carbon::now()->format('Y-m-d'));

        // Get selected team, pesticide and place filters
        $selectedTeam = $request->input('team_id');
        $selectedPesticide = $request->input('pesticide_id');
        $selectedPlace = $request->input('place_id');

        // Get the consumption data
        $consumptionData = $this->getPesticideConsumptionData($startDate, $endDate, $selectedTeam, $selectedPesticide, $selectedPlace);
     ...
     */
    private function getPesticideConsumptionData($startDate, $endDate, $teamId = null, $pesticideSlug = null, $placeId = null)
    {
        // Get all visit reports within the date range
        $query = VisitReport::with(['visit.team', 'visit.branch'])
            ->whereHas('visit', function ($q) use ($startDate, $endDate, $teamId, $placeId) {
        ...
        foreach ($reports as $report) {
            ...
            $place = $report->visit->branch ? $report->visit->branch->branch_name : 'Main Location';
            ...
            foreach ($pesticidesUsed as $pesticide) {
                ...
                // Add place data
                if (!isset($consumptionData[$team->id]['pesticides'][$pesticide]['places'][$place])) {
                    $consumptionData[$team->id]['pesticides'][$pesticide]['places'][$place] = 0;
                }
                $consumptionData[$team->id]['pesticides'][$pesticide]['places'][$place] += $quantity;
