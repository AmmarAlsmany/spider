...
<div class="page-content">
    ...
    <div class="row">
        ...
        <div class="mb-4 col-12">
            <div class="card border-0 border-start border-4 border-{{ 
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="widgets-icons-2 bg-gradient-{{ 
                            ...
                            ($contract->contract_status == 'active' ? 'primary' :
                            ($contract->contract_status == 'completed' ? 'info' : 'secondary')))) 
                        }} text-white ms-auto"><i class="bx bx-file"></i>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12 col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4 row">
                        <div class="col-md-6">
                            ...
                            <div class="mb-2">
                                ...
                                <span class="ms-2">{{ __('contract_details.basic_details.warranty_months', ['months' => $contract->warranty]) }}</span>
                            </div>
                            <div class="mb-2">
                                <strong>{{ __('contract_details.basic_details.number_of_visits') }}:</strong>
                                <span class="ms-2">{{ __('contract_details.basic_details.visits_count', ['count' => $contract->number_of_visits]) }}</span>
                            </div>
                            <div class="mb-2">
                                <strong>{{ __('contract_details.basic_details.start_date') }}:</strong>
                                <span class="ms-2">{{ \Carbon\Carbon::parse($contract->contract_start_date)->format('M d, Y') }}</span>
                            </div>
                            <div class="mb-2">
                                <strong>{{ __('contract_details.basic_details.end_date') }}:</strong>
                                <span class="ms-2">{{ \Carbon\Carbon::parse($contract->contract_end_date)->format('M d, Y') }}</span>
                            </div>
                            <div class="mb-2">
                                <strong>{{ __('contract_details.basic_details.multiple_branches') }}:</strong>
                                <span class="ms-2">{{ $contract->is_multi_branch === 'yes' ? __('contract_details.basic_details.yes') : __('contract_details.basic_details.no') }}</span>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <h6 class="mb-3 text-secondary">{{ __('contract_details.financial_information.title') }}</h6>
                            <div class="mb-2">
                            ...
                            <div class="mb-2">
                                ...
                                <span class="ms-2">{{ $contract->number_Payments ? $contract->number_Payments : '1' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-4 row">
                        <div class="col-12">
                            <h6 class="mb-3 text-secondary">{{ __('contract_details.branch_information.title') }}</h6>
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td> <a href="google.com/maps/search/?api=1&query={{ urlencode($branch->branch_address) }}" target="_blank">{{ $branch->branch_address }}</a></td>
                                            <td>{{ $branch->branch_city }}</td>
                                            <td>
                                                <strong>{{ __('contract_details.branch_information.manager') }}:</strong> {{ $branch->branch_manager_name }}<br>
                                                <strong>{{ __('contract_details.branch_information.phone') }}:</strong> <a href="tel:{{ $branch->branch_manager_phone }}">{{ $branch->branch_manager_phone }}</a><br>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            @if($contract->payments && $contract->payments->count() > 0)
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tfoot>
                                            <tr class="bg-light">
                                                <td colspan="1"><strong>{{ __('contract_details.payment_information.summary') }}</strong></td>
                                                <td><strong>{{ number_format($contract->payments->sum('payment_amount'), 2) }} SAR</strong></td>
                                                <td colspan="3">
                                                    <div class="d-flex justify-content-between">
                                                        <span>
                                                            <i class="bx bx-check text-success"></i> {{ __('contract_details.payment_information.paid') }}: 
                                                            {{ number_format($contract->payments->where('payment_status', 'paid')->sum('payment_amount'), 2) }} SAR
                                                        </span>
                                                        <span>
                                                            <i class="bx bx-time text-warning"></i> {{ __('contract_details.payment_information.pending') }}: 
                                                            {{ number_format($contract->payments->where('payment_status', 'unpaid')->sum('payment_amount'), 2) }} SAR
                                                        </span>
                                                        <span>
                                                            <i class="bx bx-x text-danger"></i> {{ __('contract_details.payment_information.overdue') }}: 
                                                            {{ number_format($contract->payments->where('payment_status', 'overdue')->sum('payment_amount'), 2) }} SAR
                                                        </span>
                                                    </div>
                                                </td>
                                            </tr>
                                ...
                                <div class="mt-3">
                                    ...
                                    <div class="mt-2 text-muted small">
                                        {{ number_format($paidAmount, 2) }} SAR {{ __('contract_details.payment_progress.paid_out_of') }} {{ number_format($totalAmount, 2) }} SAR
                                    </div>
                                </div>
                            @else
                                <div class="py-4 text-center">
                                    <i class="bx bx-money text-muted" style="font-size: 48px;"></i>
                                    <p class="mt-2">{{ __('contract_details.payment_information.no_payment_records') }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <h6 class="mb-3 text-secondary">{{ __('contract_details.contract_description.title') }}</h6>
                            <p class="text-muted">{{ $contract->contract_description ?: __('contract_details.contract_description.no_description') }}</p>
                    ...
                    @if($contract->contract_status == 'pending')
                        <div class="gap-2 d-grid">
                            <form action="{{ route('client.contract.approve', $contract->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="mb-2 btn btn-success w-100" {{ $contract->updateRequests->count() > 0 && $contract->updateRequests->first()->response === null ? 'disabled' : '' }}>
                                    <i class="bx bx-check me-1"></i> {{ __('contract_details.actions.approve_contract') }}
                                </button>
                            </form>
                            <button type="button" class="mb-2 btn btn-danger w-100" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $contract->id }}" {{ $contract->updateRequests->count() > 0 && $contract->updateRequests->first()->response === null ? 'disabled' : '' }}>
                                <i class="bx bx-x me-1"></i> {{ __('contract_details.actions.not_approve_contract') }}
                            </button>
                            <button type="button" class="btn btn-warning w-100" data-bs-toggle="modal" data-bs-target="#updateRequestModal{{ $contract->id }}">
                                <i class="bx bx-edit me-1"></i> {{ __('contract_details.actions.request_changes') }}
                            </button>
                        </div>
                    @elseif($contract->contract_status == 'approved')
                        <div class="gap-2 d-grid">
                            <button type="button" class="mb-2 btn btn-primary w-100" data-bs-toggle="modal" data-bs-target="#updateRequestModal{{ $contract->id }}">
                                <i class="bx bx-edit me-1"></i> {{ __('contract_details.actions.request_modification') }}
                            </button>
                            <a href="{{ route('client.show.payment.details', $contract->id) }}" class="btn btn-info w-100">
                ...
                <div class="card-body">
                    <div class="mb-3 d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <p class="mb-0 text-muted small">{{ __('contract_details.sales_representative.sales_agent') }}</p>
                        </div>
                    </div>
                    @if($contract->salesRepresentative->email)
                    <div class="mb-2">
                        <strong><i class="bx bx-envelope me-1"></i> {{ __('contract_details.sales_representative.email') }}:</strong>
                        <a href="mailto:{{ $contract->salesRepresentative->email }}"><span class="ms-2">{{ $contract->salesRepresentative->email }}</span></a>
                    </div>
                    @endif
                    @if($contract->salesRepresentative->phone)
                    <div class="mb-2">
                        <strong><i class="bx bx-phone me-1"></i> {{ __('contract_details.sales_representative.phone') }}:</strong>
                        <a href="tel:{{ $contract->salesRepresentative->phone }}"><span class="ms-2">{{ $contract->salesRepresentative->phone }}</span></a>
                    </div>
                    @endif
                </div>
            </div>
