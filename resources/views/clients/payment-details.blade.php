...
@section('content')
    <div class="page-content">
        ...
        <div class="mb-3 page-breadcrumb d-none d-sm-flex align-items-center">
            <div class="breadcrumb-title pe-3">{{ __('payment_details.payment_details') }}</div>
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="p-0 mb-0 breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a
                                href="{{ route('client.contract.details', $contract->id) }}">{{ __('payment_details.breadcrumb.contract_number', ['number' => $contract->contract_number]) }}</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">{{ __('payment_details.payment_details') }}
                        </li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="border-4 card border-start border-primary">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                <p class="mb-0 text-secondary">{{ __('payment_details.contract_value') }}</p>
                            ...
                            <div class="ms-auto">
                                <div class="row g-3">
                                    <div class="col-auto">
                                        <div class="text-end">
                                            <p class="mb-0 text-success">{{ __('payment_details.paid_amount') }}</p>
                                    ...
                                    <div class="col-auto">
                                        <div class="text-end">
                                            ...
                                            <h5 class="my-1 text-danger">
                                                {{ number_format(
                                                    $contract->payments->where('payment_status', 'unpaid')->where('due_date', '<', now())->sum('payment_amount'),
                                                    2,
                                                ) }}
                                                {{ __('payment_details.summary.currency') }}</h5>
                                        </div>
                                            ...
                                            <td>
                                                @if ($payment->payment_status != 'paid')
                                                    ...
                                                    <div class="gap-2 d-flex align-items-center">
                                                        ...
                                                        <button type="button" class="btn btn-sm btn-warning"
                                                            data-bs-toggle="modal" data-bs-target="#postponePaymentModal"
                                                ...
                                                <button class="gap-1 btn btn-sm btn-warning d-flex align-items-center"
                                                    ...
                                                    <i class="bx bx-receipt"></i> <span class="d-none d-sm-inline">
                                                        {{ __('payment_details.actions.view_invoice') }}
                                                    </span>
                                                </button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="py-4 text-center">
                                                ...
                                                <p class="mt-2">{{ __('payment_details.payment_schedule.no_payments') }}
                                                </p>
                                            </td>
                                        </tr>
                                    @endforelse
                ...
                <form action="{{ route('client.payment.postpone', $contract->id) }}" method="POST">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('payment_details.postpone_modal.cancel') }}</button>
                        <button type="submit"
                            class="btn btn-warning">{{ __('payment_details.postpone_modal.submit') }}</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="paymentDetailsModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        data-bs-dismiss="modal">{{ __('payment_details.payment_details_modal.close') }}</button>
                </div>
            </div>
        </div>
    </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (postponePaymentModal) {
                    postponePaymentModal.addEventListener('show.bs.modal', function(event) {
                        ...
                        this.querySelector('#current_payment_date').value = new Date(currentDate)
                            .toLocaleDateString();
                        this.querySelector('#payment_amount').value = Number(paymentAmount).toLocaleString(
                            undefined, {
                        ...
                        fetch(`/client/payment/${paymentId}/details`)
                            ...
                            .catch(error => {
                                contentDiv.innerHTML =
                                    '{{ __('payment_details.payment_details_modal.error') }}';
                            });
                    });
                }
            });
