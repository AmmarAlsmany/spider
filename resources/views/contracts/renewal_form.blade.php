        ...
        <form action="{{ route('contract.renewal.process', $contract->id) }}" method="POST" id="contract-form">
                    ...
                    <div class="col-md-4">
                        <div class="mb-3 form-group">
                            ...
                            <input type="number" class="form-control @error('warranty') is-invalid @enderror" id="warranty"
                                name="warranty" value="{{ $contract->warranty }}" min="0" required>
                            @error('warranty')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3 form-group">
                            ...
                            <input type="number" class="form-control @error('number_of_visits') is-invalid @enderror"
                                id="number_of_visits" name="number_of_visits" value="{{ $contract->number_of_visits }}" min="1"
                                required>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3 form-group">
                            ...
                            <input type="number" class="form-control @error('contract_price') is-invalid @enderror"
                                id="contract_price" name="contract_price" value="{{ $contract->contract_price / 1.15 }}" min="0"
                                step="0.01" required>
                            <small class="text-muted">VAT (15%): <span id="vatAmount">{{ number_format($contract->contract_price * 0.15 / 1.15, 2) }}</span> SAR</small>
                            <small class="d-block text-muted">Total with VAT: <span id="totalWithVat">{{ number_format($contract->contract_price, 2) }}</span> SAR</small>
            ...
            <fieldset class="mb-4">
                ...
                <div class="alert alert-info">
                    <i class="fas fa-info-circle me-2"></i>
                    This contract has {{ $contract->branchs->count() }} {{ $contract->branchs->count() == 1 ? 'branch' : 'branches' }}. All branch information will be copied to the new contract.
                </div>
                    ...
                    <div class="mb-4 card branch-card" data-branch-id="{{ $branch->id }}">
                        <div class="card-header bg-light d-flex justify-content-between align-items-center">
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input branch-include" type="checkbox" id="include_branch_{{ $branch->id }}" name="include_branches[]" value="{{ $branch->id }}" checked>
                                <label class="form-check-label" for="include_branch_{{ $branch->id }}">Include in renewal</label>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3 form-group">
                                            ...
                                            <select class="form-select" id="branch_city_{{ $branch->id }}" name="branch_data[{{ $branch->id }}][branch_city]">
                                                ...
                                                <option value="{{ $city }}" {{ $branch->branch_city == $city ? 'selected' : '' }}>{{ $city }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div class="mt-3 text-end">
                                <button type="button" class="btn btn-outline-primary btn-sm toggle-edit-branch" data-branch-id="{{ $branch->id }}">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3 form-group">
                            ...
                            <select class="form-select @error('payment_type') is-invalid @enderror" id="payment_type"
                                ...
                                <option value="prepaid" {{ $contract->payment_type == 'prepaid' ? 'selected' : '' }}>Prepaid (Full payment upfront)</option>
                                <option value="postpaid" {{ $contract->payment_type == 'postpaid' ? 'selected' : '' }}>Postpaid (Installments)</option>
                            </select>
                            @error('payment_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6 payment-details" id="postpaid-details" style="{{ $contract->payment_type == 'postpaid' ? '' : 'display: none;' }}">
                        <div class="mb-3 form-group">
                            ...
                            <input type="number" class="form-control @error('number_of_payments') is-invalid @enderror"
                                id="number_of_payments" name="number_of_payments" value="{{ $contract->number_Payments ?? 3 }}" min="1"
                                max="12">
