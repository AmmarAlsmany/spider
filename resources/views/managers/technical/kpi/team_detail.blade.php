                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <form action="{{ route('technical.team.kpi.detail', $team->id) }}" method="GET" class="mt-3 mt-md-0">
                                    <div class="row">
                                        <div class="col-md-5">
                                            <div class="mb-3">
                            ...
                            <div class="col-md-6">
                                <div class="mb-4">
                                    ...
                                    <div class="progress" style="height: 20px;">
                                        <div class="progress-bar bg-success" role="progressbar" style="width: {{ $kpiData[$team->id]['completion_rate'] }}%;" 
                                            aria-valuenow="{{ $kpiData[$team->id]['completion_rate'] }}" aria-valuemin="0" aria-valuemax="100">
                                            {{ $kpiData[$team->id]['completion_rate'] }}%
                                        </div>
                                    </div>
                                    <div class="mt-2 text-muted small">
                                        Completed {{ $kpiData[$team->id]['completed_visits'] }} of {{ $kpiData[$team->id]['total_visits'] }} visits
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-4">
                                    ...
                                    <div class="progress" style="height: 20px;">
                                        <div class="progress-bar bg-info" role="progressbar" style="width: {{ $kpiData[$team->id]['report_rate'] }}%;" 
                                            aria-valuenow="{{ $kpiData[$team->id]['report_rate'] }}" aria-valuemin="0" aria-valuemax="100">
                                            {{ $kpiData[$team->id]['report_rate'] }}%
                                        </div>
                                    </div>
                                    <div class="mt-2 text-muted small">
                                        Submitted {{ $kpiData[$team->id]['reports_created'] }} of {{ $kpiData[$team->id]['completed_visits'] }} reports
                                    </div>
                                </div>
                            </div>
                        </div>
                                                    ...
                                                    <div class="flex-grow-1">
                                                        ...
                                                        <div class="progress" style="height: 5px;">
                                                            <div class="progress-bar bg-primary" role="progressbar" 
                                                                style="width: {{ ($count / array_sum($kpiData[$team->id]['top_insects'])) * 100 }}%;" 
                                                                aria-valuenow="{{ $count }}" aria-valuemin="0" 
                                                                aria-valuemax="{{ array_sum($kpiData[$team->id]['top_insects']) }}"></div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="flex-grow-1">
                                                        ...
                                                        <div class="progress" style="height: 5px;">
                                                            <div class="progress-bar bg-success" role="progressbar" 
                                                                style="width: {{ ($count / array_sum($kpiData[$team->id]['top_pesticides'])) * 100 }}%;" 
                                                                aria-valuenow="{{ $count }}" aria-valuemin="0" 
                                                                aria-valuemax="{{ array_sum($kpiData[$team->id]['top_pesticides']) }}"></div>
                                                        </div>
                                                    </div>
                                                    <span class="ms-2">{{ $count }}</span>
                                                </div>
                            ...
                            <table class="table mb-0 table-centered table-nowrap">
                                ...
                                <tbody>
                                    @forelse($visitData as $visit)
                                        <tr>
                                            <td>{{ \Carbon\Carbon::parse($visit->visit_date)->format('d M Y') }} {{ $visit->visit_time }}</td>
                                            <td>
                                                <a href="{{ route('technical.contract.show', $visit->contract_id) }}">
                                                    {{ $visit->contract->contract_number }}
                                                </a>
                                                <div class="small text-muted">{{ $visit->contract->customer->name }}</div>
                                            </td>
                                            <td>{{ $visit->branch->branch_name }} <span class="text-muted">{{ $visit->branch->branch_address }}</span></td>
                                            <td>
                                            ...
                                            <td>
                                                @if($visit->report)
                                                    <a href="{{ route('technical.visit.report.view', $visit->id) }}" class="btn btn-sm btn-soft-primary">
                                            ...
                                            <td>
                                                @if($visit->report && $visit->report->time_in && $visit->report->time_out)
                                                    {{ \Carbon\Carbon::parse($visit->report->time_in)->diffInMinutes(\Carbon\Carbon::parse($visit->report->time_out)) }} min
                                                @else
                                                    -
                                                @endif
                                            </td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="7" class="text-center">No visits found for the selected period</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
